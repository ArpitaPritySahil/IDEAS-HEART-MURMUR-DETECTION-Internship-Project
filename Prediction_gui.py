...
def get_insights(dataset_file_name,dataset_df):
    Prediction_grid = grid([1], vertical_align="center")
...
def Prediction_gui():
    ...
    if dataset_file_name is not None:
        ...
        if len(model_type) > 0 and model_name == model_type[0] and pred_button:
            get_insights(dataset_file_name,dataset_df)
        elif len(model_type) > 1 and model_name==model_type[1] and pred_button:
